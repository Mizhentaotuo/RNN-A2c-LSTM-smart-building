...
def normalized_columns_initializer(weights, std=1.0):
    out = torch.randn(weights.size(), requires_grad = True)
    out_norm = out.clone()
    out_norm *= std / torch.sqrt(torch.reshape(out.pow(2).sum(1), (out.shape[0], 1)).expand_as(out)) # thanks to this initialization, we have var(out) = std^2
    return out_norm
...
class ActorCritic(torch.nn.Module):
    ...
    def __init__(self, num_inputs, action_space): # num_inputs, input data [1, 2, 3]
        ...
        self.lstm = nn.LSTMCell(num_inputs, self.hidden_layer) # making an LSTM (Long Short Term Memory) to learn the temporal properties of the input - we obtain a big encoded vector S of size 256 that encodes an event of the game
        num_outputs = action_space.size # getting the number of possible actions
        self.critic_linear = nn.Linear(self.hidden_layer, 1) # full connection of the critic: output = V(S)
        self.actor_linear = nn.Linear(self.hidden_layer, num_outputs) # full connection of the actor: output = Q(S,A)
    ...
    def forward(self, inputs):
        ...
        x = inputs # x.view(-1, 32 * 3 * 3) # flattening the last convolutional layer into this 1D vector x
